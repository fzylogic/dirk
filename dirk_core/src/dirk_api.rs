...
async fn full_scan(
        ...
        .filter(move |p| !cached_sums.contains(&p.sha1sum))
    {
        let s3 = s2.clone();
        results.push(
            tokio::task::spawn_blocking(move || req.process(&s3))
                .await
                .unwrap(),
        );
    }
...
pub async fn get_db() -> Result<DatabaseConnection, DbErr> {
    Database::connect(
        &env::var("DIRK_DATABASE_URL")
            .expect("Need to set DIRK_DATABASE_URL to point to your database"),
    )
    .await
}
